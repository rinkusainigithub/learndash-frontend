...
<?php
	...
	if(isset($_GET['quest_id'])){
		...
		$quest_cont = $quest->post_content;
		$quest_course_id = get_post_meta( $quest_id, 'course_id', true );
		$quest_lesson_id = get_post_meta( $quest_id, 'lesson_id', true );
		$quest_quiz_id = get_post_meta( $quest_id, 'quiz_id', true );
		$course_lesson_ids = learndash_course_get_steps_by_type( $quest_course_id, 'sfwd-lessons' );
		$course_quizzes_ids = learndash_course_get_steps_by_type( $quest_course_id, 'sfwd-quiz' );
	$out = "";
	$outquizess = "";
	foreach($course_lesson_ids as $course_lesson_id){
		if($quest_lesson_id == $course_lesson_id){
			$selected = "Selected";
		}
		else{
			$selected = "";
	...
	foreach($course_quizzes_ids as $course_quiz_id){
		if($quest_quiz_id == $course_quiz_id){
			$selected = "Selected";
		}
		else{
			$selected = "";
		}
		$outquizess .= '<option value="'.$course_quiz_id.'" '.$selected.'>'.get_the_title($course_quiz_id).'</option>';
	}
		$quest_lesson = get_post( $quest_lesson_id );
	}
...
<form id="cst_question_form" novalidate action="action.php" name="cst_question_form" class="class_children_data1" method="post">
    <div class="formcontrol">
        ...
        <select name="select_cource" id="cst_course_id">
            ...
            <?php
                ...
                {
            ?>
                <option value="<?php echo $course->ID ?>" <?php if($quest_course_id == $course->ID) echo "selected"; ?>><?php echo $course->post_title  ?></option>
            <?php
                }
            ?>
        </select>
    ...
    <div>
        <div class="repeater"> 
			...
			<?php
		$qquets = new LDLMS_Quiz_Questions($quest_quiz_id);
		$quests = $qquets->get_questions();
		if($quests && isset($quest_quiz_id)){
		global $wpdb;
	  ...
	  <div class="form-group">
    <label for="lName"><?php if($custom_labels['question']!= '') echo $custom_labels['question']; else "Question"; ?></label>
	<input type="text" class="form-control dyna_remo" id="essay_quest_title" name="essay_quest_title" placeholder="<?php if($custom_labels['question']!= '') echo $custom_labels['question']; else "Question"; ?> title" value="<?php if($data['_answerType'] == "essay") echo $data['_title']; ?>">
		</div> 
	<div class="answer-group">
                            <div class="form-group">
	...
	<input type="hidden" id="cst_quest_id" name="cst_quest_id" value="<?php if(isset($_GET['quest_id'])){  echo $_GET['quest_id']; } ?>">
