...
$version = rand(10,10000);
  ...
  wp_register_script('slidesjs_core', plugins_url('/assets/js/tinymce.min1.js', __FILE__),array("jquery"));
  ...
  wp_register_script('slidesjs_core3', plugins_url('/assets/js/repeater.js', __FILE__),array("jquery"),$version);
  ...
  $custom_page_links = get_option('learndash_frontend_page_links');
	...
	// }
	if(!empty($custom_page_links['frontned_class_creation_page'])){
		$cst_frnt_page = get_site_url().get_option('cst_frnt_page');
	}
	else{
		$cst_frnt_page = get_site_url()."/my-account/create-class/";
	}
    if(!empty($custom_page_links['frontned_course_show_all'])){
		$cst_courses_show_all = $custom_page_links['frontned_course_show_all'];
	}
  $localize_vars  = apply_filters( 'dokan_admin_localize_param', array(
		'ajaxurl'      => admin_url( 'admin-ajax.php' ),
		'cstsideurl'      => get_site_url(),
		'all_classes' => $cst_frnt_page,
		'cst_courses_show_all' => $cst_courses_show_all
	) );
...
function cst_do_register($username, $first_name, $last_name, $email, $password, $mobile, $complete_address, $website, $facebook, $linkedin)
    {
        ...
        $userdata = array(
            ...
            if (!is_wp_error($register_user)) {
				wp_update_user( array ('ID' => $register_user) ) ;
...
function cst_learndash_lesson_exists($title, $course_id, $post_type){
	global $wpdb;
	if ( empty( $title ) ) {
		return false;
	}
	
	$posts = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE post_title = %s AND post_type= %s", $title, $post_type));
...
function cst_create_lesson(){
	...
	$class_id = $_SESSION['cst_class_creation'];
	 ...
	 foreach($values as $key => $value){
		...
		else if(!empty($hmtitle)){
		
		$lession_id = cst_learndash_lesson_exists($hmtitle, $class_id, 'sfwd-lessons');
			$module = array(
			'ID'           => $lession_id,
			'post_title'  => $mtitle,
			'post_content'  => $module_desc,
		...
		if($assigment_check=='Yes')
		{
			...
			$lessionpostdate['sfwd-lessons_lesson_assignment_upload']='';
			$lessionpostdate['sfwd-lessons_assignment_upload_limit_extensions']='';
			$lessionpostdate['sfwd-lessons_assignment_upload_limit_size']='';
		}
...
foreach($course_lesson_ids as $lessid){
	if(!in_array($lessid, $cst_modules_creation)){
		wp_delete_post($lessid, true);
	}
}
...
function cst_assign_class_price() {
	...
	$data = array('sfwd-courses_wcf_course_template'=> "none", 'sfwd-courses_course_prerequisite_enabled'=> "", 'sfwd-courses_course_prerequisite'=>"", 'sfwd-courses_course_materials_enabled'=>"", 'sfwd-courses_course_lesson_order_enabled'=>"", 'sfwd-courses_course_materials'=>"", 'sfwd-courses_certificate'=>"", 'sfwd-courses_course_disable_content_table'=> $disable_content, 'sfwd-courses_course_price_type'=> $access_mode, 'sfwd-courses_course_lesson_per_page'=>"", 'sfwd-courses_course_lesson_per_page_custom'=>"", 'sfwd-courses_course_topic_per_page_custom'=> "", 'sfwd-courses_course_lesson_orderby'=> "", 'sfwd-courses_course_lesson_order'=>"", 'sfwd-courses_course_access_list_enabled'=>"");
...
function cst_get_lessons(){
	...
	if($custom_labels['lesson']!= '') {
		$out = "<option>Select ".$custom_labels['lesson']."</option>";
	} else {
...
function cst_save_topic(){
	...
	if(isset($params['topic_update'])){
		// Update post 37
		$my_post = array(
		'ID'           => $params['topic_update'],
	...
	$ld_course_steps = get_post_meta($params["select_cource"], 'ld_course_steps', true);
	$lessid = $params["select_lesson"];
	$ld_course_steps['h']['sfwd-lessons'][$lessid]['sfwd-topic'][$the_post_id] = array('sfwd-quiz'=>array());
    
    $ld_course_steps['t']['sfwd-topic'][]=$the_post_id;
	$ld_course_steps['r']['sfwd-topic:'.$the_post_id]=array(0=>"sfwd-lessons:".$lessid);
	$ld_course_steps['l'][]="sfwd-topic:".$the_post_id;
    
	update_post_meta($params["select_cource"], 'ld_course_steps', $ld_course_steps );
...
function cst_save_quiz(){
	...
	//$quiz_desc = strip_tags($quiz_desc);
	//var_dump($quiz_title);
	if(isset($values['cst_quiz_id']) && !empty($values['cst_quiz_id'])){
		$quizdtl = array(
		'ID'            => $values['cst_quiz_id'],
		'post_title'    => $quiz_title,
		'post_content'  => $quiz_desc,
		'post_status'   => 'publish',
		'post_author'   => get_current_user_id(),
		...
		$quiz_id = wp_update_post( $quizdtl );
		$qquets = new LDLMS_Quiz_Questions($values['cst_quiz_id']);
	...
	foreach ($questions as $question) {
		...
		$question_id = wp_insert_post( $quesdtl );
		update_post_meta($question_id, 'quiz_id', $quiz_id);
		$ld_course_steps = get_post_meta($cst_course_id, 'ld_course_steps', true);
		$ld_course_steps['h']['sfwd-lessons'][$cst_lesson_id]['sfwd-quiz'][$quiz_id] = array();
		update_post_meta($cst_course_id, 'ld_course_steps', $ld_course_steps );
		update_post_meta($question_id, 'course_id', $cst_course_id);
		update_post_meta($question_id, 'lesson_id', $cst_lesson_id);
		update_post_meta($question_id, 'question_type', $quest_type); 
		update_post_meta($question_id, '_sfwd-question', array('sfwd-question_quiz'=>strval($quiz_id)));
...
add_action('wp_ajax_cst_save_tags_categories', 'cst_save_tags_categories');
function cst_save_tags_categories() {
	$values = $_POST['val'];
	foreach ($values as $value) {
		...
		$status = json_decode($value["status"]);
		global $wpdb;
		$term_relationshipsTb = $wpdb->prefix . "term_relationships";
		if($status) {
			$deleteQry = "DELETE FROM `$term_relationshipsTb` WHERE `object_id` = %d and `term_taxonomy_id` = %d";
			$deletetagQry = $wpdb->prepare($deleteQry, $object_id, $term_taxonomy_id);
			var_dump($deletetagQry);
			$wpdb->query($deletetagQry);
		} else {
			
			$termQuery = $wpdb->prepare("INSERT INTO `$term_relationshipsTb` (`object_id`, `term_taxonomy_id`) values (%s, %s)", $object_id, $term_taxonomy_id);
			$wpdb->query($termQuery);
			$term_id = $wpdb->insert_id;
		}
	}
}

add_action('wp_ajax_cst_save_learndash_category', 'cst_save_learndash_category');
function cst_save_learndash_category() {
	$values = $_POST['val'];
	$name = $values['category_value'];
	global $wpdb;
	$termsTb = $wpdb->prefix . "terms";
	$termQuery = $wpdb->prepare("INSERT INTO `$termsTb` (`name`, `slug`) values (%s, %s)", $name, $name);
	$wpdb->query($termQuery);
	$term_id = $wpdb->insert_id;

	$term_taxonomyTb = $wpdb->prefix . "term_taxonomy";
	$term_taxonomyQuery = $wpdb->prepare("INSERT INTO `$term_taxonomyTb` (`term_id`, `taxonomy`, `count`) values (%s, %s, %s)", $term_id, 'category', 1);
	$wpdb->query($term_taxonomyQuery);
}
...
add_shortcode('cst_learndash_tags', 'cst_learndash_tags');
function cst_learndash_tags(){
wp_enqueue_style( 'astra-bootstrap-css' );
		wp_enqueue_style( 'astra-conditional-css' );
		wp_enqueue_script( 'astra-bootstrap-js' );
	require_once( plugin_dir_path( __FILE__ ).'/template-parts/create-tags.php');
}

add_shortcode('cst_learndash_categories', 'cst_learndash_categories');
function cst_learndash_categories(){
wp_enqueue_style( 'astra-bootstrap-css' );
		wp_enqueue_style( 'astra-conditional-css' );
		wp_enqueue_script( 'astra-bootstrap-js' );
	require_once( plugin_dir_path( __FILE__ ).'/template-parts/create-categories.php');
}

add_action('wp_ajax_cst_save_tag', 'cst_save_tag');
function cst_save_tag() {
	$values = $_POST['val'];
	$name = $values['tag_value'];
	global $wpdb;
	$termsTb = $wpdb->prefix . "terms";
	$termQuery = $wpdb->prepare("INSERT INTO `$termsTb` (`name`, `slug`) values (%s, %s)", $name, $name);
	$wpdb->query($termQuery);
	...
	$term_taxonomyTb = $wpdb->prefix . "term_taxonomy";
	$term_taxonomyQuery = $wpdb->prepare("INSERT INTO `$term_taxonomyTb` (`term_id`, `taxonomy`, `count`) values (%s, %s, %s)", $term_id, 'post_tag', 1);
	$wpdb->query($term_taxonomyQuery);
}

require_once ABSPATH . 'wp-content/plugins/sfwd-lms/includes/settings/class-ld-settings-sections.php';
require_once plugin_dir_path( __FILE__ ) . '/setting/class-ld-settings-section-frontend-page-links.php';