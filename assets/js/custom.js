jQuery(document).ready(function ($) {
		...
		classtable.find("tr>td>select").each(function() {
				...
				for(var i=0; i< options.length; i++) {
					...
					intializeMultiSelect.setCheckBoxClick(optionsValue, function(target, args){
						if(id.includes('testSelect1')) {
							optionChange(target, args, 'assignedGroups');
						} else if(id.includes('tagsSelect1')) {
							optionChange(target, args, 'assignedTags');
						} else if (id.includes('categoriesSelect1')) {
							optionChange(target, args, 'assignedCategories');
						}
					});
				}
				intializeMultiSelect.setCheckBoxClick("checkboxAll", function(target, args){
					var parentID = target.parentElement.parentElement.parentElement.id;
					if(parentID.includes('testSelect1')) {
						selectAllChange(target, args, 'assignedGroups')
					} else if (parentID.includes('tagsSelect1')) {
						selectAllChange(target, args, 'assignedTags')
					} else if (parentID.includes('categoriesSelect1')) {
						selectAllChange(target, args, 'assignedCategories');
					}
				});
	...
	if(url.includes("step3")) {
		var controls = ["#previousStep3", ".cst_preview"];
	...
	jQuery('.clsdelete').on('click', function (e) {
		e.preventDefault();
		...
		jQuery.ajax({
			...
			success: function (responce) {
				Swal.fire({
                    ...
                    window.location.replace(ajax_vars.cst_courses_show_all);
				})
			}
		});
	});
	...
	$(".next").click(function () {
		...
		setTimeout(function(){
	var cst_del_w = jQuery(".cst_del").width();
	 cst_del_w = cst_del_w +40;
	console.log("cl : "+ cst_del_w);
jQuery('.cst_addmd').css({"position":"relative"});
jQuery('.cst_addmd').css({"left":cst_del_w+"px"} );
	},300);
	});
	
	setTimeout(function(){
	var cst_del_w = jQuery(".del_ques").width();
	 cst_del_w = cst_del_w +50;
	console.log("cl : "+ cst_del_w);
jQuery('.add_ques').css({"left":cst_del_w+"px"} );
	},300);
	...
	$(document).on('submit', 'form[name="cst_topic"]', function (e) {
		e.preventDefault();
		var dataToSend = getJsonData('topic');
		var isFormValid = checkValidations(dataToSend, "topic");
		if (isFormValid) {
	...
	$(document).on('submit', 'form[name="cst_quiz_form"]', function (e) {
		e.preventDefault();
		var dataToSend = getJsonData('quiz');
	...
	$(document).on('submit', 'form[name="cst_question_form"]', function (e) {
		e.preventDefault();
		var dataToSend = getJsonData('question');
	...
	function getJsonData(context) {
		...
		if(context == "quiz" || context == "question"){
			dataToSend.cst_quiz_title = jQuery("#quiztitle").val(),
			dataToSend.cst_quiz_description = jQuery("#kv_frontend_editor").val(),
			dataToSend.cst_quiz_id = jQuery("#cst_quiz_id").val(),
			...
			dataToSend.cst_topic_id = jQuery("#topictitle").val();
		}
		return dataToSend;
	}
	...
	function checkValidations(dataToSend, validationFor = "") {
		...
		else if (validationFor == "topic") {
			if (!dataToSend.cst_topic_id) {
				validations.push("Topic title is required.")
			}
		}
	...
	$(document).on('submit', 'form[name="cst_tag"]', function (e) {
		e.preventDefault();
		jQuery("#tagsValidation").css('display','none');
		var isFormValid = jQuery("#tag").val();
		if (isFormValid) {
			var dataToSend = {
				tag_value: isFormValid
			};
			postRequest(dataToSend, 'cst_save_tag', 'Tag has been created.');
		} else {
			jQuery("#tagsValidation").css('display','block');
		}
	})
	
	$(document).on('submit', 'form[name="cst_category"]', function (e) {
		e.preventDefault();
		jQuery("#categoryValidation").css('display','none');
		var isFormValid = jQuery("#categoryId").val();
		if (isFormValid) {
			var dataToSend = {
				category_value: isFormValid
			};
			postRequest(dataToSend, 'cst_save_learndash_category', 'Category has been created.');
		} else {
			jQuery("#categoryValidation").css('display','block');
		}
	})
...
function openCity(evt, cityName) {
  ...
  tabcontent = document.getElementsByClassName("cst_tabcontent");
  for (i = 0; i < tabcontent.length; i++) {
    tabcontent[i].style.display = "none";
  }
  tablinks = document.getElementsByClassName("cst_tablinks");
...
function generateTagsOptions (optionsElements, args) {
	var tags = [];
	for(var i=0; i< optionsElements.length; i++) {
		var optionsValue = optionsElements[i].value;
		var singleTag = getTagsObject(optionsValue, args.checked);
		tags.push(singleTag);
	}
	updateTags(tags);
}

function optionChange(target, args, control) {
	if(control == 'assignedGroups') {
		var courseGroupS = [];
		var courseGroup = getCourseGroupObject(args.id, args.checked);
		courseGroupS.push(courseGroup);
		updateCourseGroup(courseGroupS);
	} else if (control == 'assignedTags' || control == 'assignedCategories') {
		var tagsData = [];
		var singleTag = getTagsObject(args.id, args.checked);
		tagsData.push(singleTag);
		updateTags(tagsData);
	}
}

function selectAllChange(target, args, controlText) {
		...
		if(parentDropDownID && parentDropDownID.length > 0 && parentDropDownID[0]) {
			...
			//	generateOptions(control, args)
			//}
			if(controlText == 'assignedGroups') {
				generateOptions(control, args);
			} else if (controlText == 'assignedTags' || controlText == 'assignedCategories') {
				generateTagsOptions(control, args)
			}
		}
	
}

function getTagsObject(value, status) {
	const tagsData = value.split("&");
	if(tagsData && tagsData.length > 2) {
		return  { term_id: tagsData[0], object_id: tagsData[1], term_taxonomy_id: tagsData[2], status: !status}
	}
	return null;
}
...
function updateTags(tags) {
	if(tags && tags.length > 0) {
		postRequest(tags, 'cst_save_tags_categories', 'Updated successfully');
	}
}
...
function open_media_uploader_image(ths) {
	...
	image_uploader.on("insert", function () {
		...
		for (var iii = 0; iii < length; iii++) {
			...
			var vhtml = "";
			if (mtype == "image") {
				vhtml = '<img src="' + audio_url + '" data-mce-src="' + audio_url + '">';
			}
